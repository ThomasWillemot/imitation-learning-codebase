        ...
        for batch in self.data_loader.get_data_batch():
            with torch.no_grad():
                ...
                if self._config.store_projected_output_on_tensorboard:
                    ...
                    x_position = 0
                    y_position = 0
                    if not (np_pred[0][0] ==0):
                        x_position = int(-np_pred[0][1] / np_pred[0][0] * 278.9/scaling_factor + 214.5/scaling_factor)
                        y_position = int(-np_pred[0][2] / np_pred[0][0] * 278.9/scaling_factor + 120.5/scaling_factor)
                    if 0<x_position<848/scaling_factor and 0<y_position<800/scaling_factor:
                        cone_circle_cv = cv2.circle(numpy_obs[0, :, :], (x_position, y_position), int(50/np_pred[0][0]/2), 1, int(2))
                        cone_circle_image_np = cone_circle_cv
                    else:
                        cone_circle_image_np = numpy_obs[0, :, :]
                    if self._config.split_losses:
                        x_position_1 = int(-np_pred[0][4] / np_pred[0][3] * 278.9/scaling_factor + 214.5/scaling_factor)
                        y_position_1 = int(-np_pred[0][5] / np_pred[0][3] * 278.9/scaling_factor + 120.5/scaling_factor)
